...
from .const import (
    ...
    CONF_CONNECTION_MQTT_CHANNEL_KEYS,
...
def _step_user_data_connection_mqtt_schema_factory(
    ...
    return vol.Schema(
        {
            vol.Required(CONF_CONNECTION_MQTT_HOST, default=host): cv.string,
            vol.Required(CONF_CONNECTION_MQTT_PORT, default=port): cv.positive_int,
            vol.Optional(CONF_CONNECTION_MQTT_USERNAME, default=username): cv.string,
            vol.Optional(CONF_CONNECTION_MQTT_PASSWORD, default=password): cv.string,
            vol.Optional(CONF_CONNECTION_MQTT_TLS, default=tls): cv.boolean,
            vol.Optional(CONF_CONNECTION_MQTT_TOPIC, default=topic): cv.string,
        }
    )


def _step_mqtt_channels_schema_factory() -> vol.Schema:
    return vol.Schema(
        {
            vol.Optional("channel_name", default="LongFast"): cv.string,
            vol.Optional("channel_key", default="AQ=="): cv.string,
            vol.Optional("add_another_channel", default=False): cv.boolean,
        }
    )
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        return self.async_show_menu(
            step_id="user",
            menu_options=["manual_tcp", "manual_bluetooth", "manual_serial", "manual_mqtt"],
        )
    ...
    async def async_step_manual_mqtt(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        self.data[CONF_CONNECTION_TYPE] = ConnectionType.MQTT.value
        errors: dict[str, str] = {}
        if user_input is not None:
            # Validate broker host
            if not user_input.get(CONF_CONNECTION_MQTT_HOST, "").strip():
                errors[CONF_CONNECTION_MQTT_HOST] = "cannot_connect"
            # Validate port
            port = user_input.get(CONF_CONNECTION_MQTT_PORT, 0)
            if not (1 <= port <= 65535):
                errors[CONF_CONNECTION_MQTT_PORT] = "cannot_connect"
    ...
    async def async_step_mqtt_channels(self, user_input: dict[str, Any] | None = None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            channel_key = user_input.get("channel_key", "").strip()

            if channel_name and channel_key:
                # Validate base64 key
                try:
                    base64.b64decode(channel_key)
                except Exception:
            ...
            if not errors:
                ...
                # Generate a deterministic unique ID from broker params
                broker_id = f"mqtt_{self.data[CONF_CONNECTION_MQTT_HOST]}_{self.data[CONF_CONNECTION_MQTT_PORT]}"
                unique_id = hashlib.md5(broker_id.encode()).hexdigest()[:8]  # noqa: S324
                await self.async_set_unique_id(f"mqtt_{unique_id}")
                self._abort_if_unique_id_configured()

                # Test connection
                try:
                    gateway_node, nodes = await validate_input_for_connection(self.hass, self.data, no_nodes=True)
                    self.gateway_node = gateway_node
                    self.nodes = nodes
                except CannotConnectError:
                    errors["base"] = "cannot_connect"
                except Exception:
                    _LOGGER.warning("Unexpected exception during MQTT test", exc_info=True)
                    errors["base"] = "unknown"
    ...
    async def async_step_reconfigure(self, user_input: dict[str, Any] | None = None) -> FlowResult:  # noqa: ARG002
        ...
        if connection_type == ConnectionType.MQTT.value:
            return await self.async_step_manual_mqtt()
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input: dict[str, Any] | None = None) -> dict[str, Any]:  # noqa: PLR0912
        ...
        # For MQTT connections, show a simplified options flow
        connection_type = self.config_entry.data.get(CONF_CONNECTION_TYPE)
        if connection_type == ConnectionType.MQTT.value:
            return await self._async_step_mqtt_options(user_input)
